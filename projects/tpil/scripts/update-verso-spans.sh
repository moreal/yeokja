...
# The extractor is shared with the other Verso book project. It depends only
# on Verso.Doc.Concrete and accepts the source root and file list as arguments.
extractor="$project_root/../fp-lean/tools/VersoSpans.lean"
...
sources=("book/TPiL.lean")
while IFS= read -r source; do
  sources+=("${source#"$upstream_root/"}")
done < <(find "$upstream_root/book/TPiL" -maxdepth 1 -type f -name '*.lean' | LC_ALL=C sort)
