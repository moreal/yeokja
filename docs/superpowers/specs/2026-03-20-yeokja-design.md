...
- `content`: 원본 텍스트
- `content_hash`: 내용 해시
- `context_hash`: 앞뒤 세그먼트 해시 조합 (context match용)
- `block_type`: heading / paragraph / list_item 등 (코드 블록은 번역 대상에서 제외)

### 번역 단위

- 저장/추적은 문장 단위
- 번역 요청 시에는 해당 문장이 속한 블록(문단) 전체를 문맥으로 LLM에 제공하되, 대상 문장을 명시
...
1. 원본 파일을 다시 파싱 → 새 세그먼트 목록 생성
2. 각 세그먼트의 `source_hash`를 기존과 비교 → 불일치하면 stale
3. `context_hash` 비교 → 내용은 같지만 문맥이 바뀌면 context_changed (재번역 우선순위 낮음)
4. 새로 추가된 세그먼트 → pending
5. 삭제된 세그먼트 → 상태 파일에서 제거

### Glossary 연동

- 번역 시 사용된 glossary 항목의 실제 매핑값을 `glossary_snapshot`에 기록
...
- `segments`: 번역 대상 문장들
- `context`: 해당 문장이 속한 블록(문단) 전체 텍스트
- `glossary`: 관련 용어 목록
...
- 429/rate limit 에러 시 exponential backoff
- 응답 헤더의 `retry-after`, `x-ratelimit-remaining` 등을 파싱하여 요청 속도 자동 조절
...
- `yeokja translate ./book/ --tui` 또는 `yeokja watch`
...
- `output`: `{dir}`, `{stem}`, `{ext}`, `{path}` 템플릿 변수
- API 키는 환경변수명 참조
